          ...
          {filteredProducts.length > 0 ? (
            <>
              <div className="mb-4 text-sm text-gray-600 font-medium">
                Showing <strong className="text-orange-500">{filteredProducts.length}</strong> results
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                {filteredProducts.map((product) => (
                  <Link to={`/products/${product.id}`} key={product.id} className="group block h-full">
                    <article className="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-lg hover:border-orange-300 transition-all duration-300 cursor-pointer h-full">
                      <div className="relative overflow-hidden bg-gray-50">
                        <img className="w-full h-auto object-cover group-hover:scale-105 transition-transform duration-300" src={product.thumbnail} alt={product.title} loading="lazy" />
                      ...
                      <div className="p-4 space-y-3 flex-1">
                        {product.brand && <span className="text-xs font-medium text-gray-500 uppercase">{product.brand}</span>}
